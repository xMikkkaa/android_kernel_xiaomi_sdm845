...
preflight_check() {
    ...
    log_info "Jobs: ${JOBS}"
    log_info "LTO: Full LTO (Clang)"
    log_info "Features: SCS, DCE"
...
show_help() {
    ...
    echo "    --dynamic    Dynamic Partition Variant (skips fstab injection)"
    echo ""
    echo "  Audio Options:"
...
apply_fstab_variant() {
    if [ "${VARIANT}" = "dynamic" ]; then
        log_step "Skipping fstab configuration for Dynamic Partitions..."
        return
    fi
