...
/*
 * Raw profile data format (LLVM 23 v10):
 *
 *	- llvm_prf_header (16 x u64 = 128 bytes)
 *	- __llvm_prf_data
 *	- __llvm_prf_cnts
 *	- __llvm_prf_bits (bitmap, LLVM 23)
 *	- __llvm_prf_names
...
static void prf_fill_header(void **buffer)
{
	...
	header->padding_bytes_after_bitmap = 0;
	header->names_size = prf_names_count();
	header->counters_delta = (u64)((char *)__llvm_prf_cnts_start - (char *)__llvm_prf_data_start);
	header->bitmap_delta = (u64)((char *)__llvm_prf_bits_start - (char *)__llvm_prf_data_start);
	header->names_delta = (u64)__llvm_prf_names_start;
	header->num_vtables = 0;
	header->vnames_size = 0;
