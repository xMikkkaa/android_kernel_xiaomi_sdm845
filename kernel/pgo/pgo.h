...
/*
 * LLVM profile data definitions for Clang 23 (LLVM INSTR_PROF_RAW_VERSION 10).
 * Adapted from llvm/include/llvm/ProfileData/InstrProfData.inc.
 */
...
/**
 * struct llvm_prf_header - represents the raw profile header (LLVM 23 v10).
 * @magic: the magic token for the file format.
 * @version: the version of the file format (10 for LLVM 23).
 * @binary_ids_size: the size of the binary IDs section.
 ...
 *   counters.
 * @num_bitmap_bytes: the size of the bitmap section in bytes.
 * @padding_bytes_after_bitmap: the number of padding bytes after the bitmap.
 * @names_size: the size in bytes of the LLVM profile section containing the
 *   counters' names.
 * @counters_delta: the offset from data section start to counters section.
 * @bitmap_delta: the offset from data section start to bitmap section.
 * @names_delta: the absolute address of the names section.
 * @num_vtables: the number of vtable entries.
 * @vnames_size: the size of the vtable names section.
...
struct llvm_prf_header {
	...
	u64 padding_bytes_after_bitmap;
	u64 names_size;
	u64 counters_delta;
	u64 bitmap_delta;
	u64 names_delta;
 ...
 * @counter_ptr: a relative pointer to the profile counter.
 * @bitmap_ptr: a relative pointer to the bitmap data.
 ...
 * @num_value_sites: the number of value profile sites (3 for LLVM 23).
 * @num_bitmap_bytes: the number of bitmap bytes for this function.
 */
...
extern u64 __llvm_prf_bits_start[];
extern u64 __llvm_prf_bits_end[];

extern struct llvm_prf_value_node __llvm_prf_vnds_start[];
extern struct llvm_prf_value_node __llvm_prf_vnds_end[];

extern char __llvm_prf_vals_start[];
extern char __llvm_prf_vals_end[];
