...
#define SUSFS_STATE_LOOKUP_LAST (1 << 0)
#define SUSFS_STATE_OPEN_LAST   (1 << 1)
		       ...
		       unsigned *seqp)
{
	...
	int err;
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	bool is_nd_state_lookup_last_and_open_last = (nd->susfs_state & (SUSFS_STATE_LOOKUP_LAST | SUSFS_STATE_OPEN_LAST));
#endif
				  ...
				  unsigned int flags)
{
	...
	bool is_nd_flags_lookup_last = current->nameidata && (current->nameidata->susfs_state & SUSFS_STATE_LOOKUP_LAST);
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (nd->path.dentry && nd->path.dentry->d_inode && susfs_is_inode_sus_path(nd->path.dentry->d_inode)) {
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	nd->susfs_state |= SUSFS_STATE_LOOKUP_LAST;
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	bool found_sus_path = false;
	bool is_nd_state_open_last = (nd->susfs_state & SUSFS_STATE_OPEN_LAST);
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	nd->susfs_state |= SUSFS_STATE_OPEN_LAST;
#endif
